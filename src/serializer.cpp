...
Serializer::Serializer(const sp::string& filename)
: handler(filename)
{
}

bool Serializer::read(const sp::string& name, ISerializable& object)
{
    return handler.read(name, object);
}

void Serializer::write(const sp::string& name, ISerializable& object)
{
    handler.write(name, object);
}

Serializer::Handler::Handler(const sp::string& filename)
: filename(filename)
{
}
...
bool Serializer::Handler::read(const sp::string& name, ISerializable& object)
{
...
void Serializer::Handler::write(const sp::string& name, ISerializable& object)
{
...
void Serializer::Handler::skipType(int type_id)
{
    ...
    case Type_Vector2f: read_index += sizeof(sp::Vector2f); break;
    case Type_Vector2d: read_index += sizeof(sp::Vector2d); break;
    case Type_Vector3f: read_index += sizeof(sp::Vector3f); break;
    case Type_Vector3d: read_index += sizeof(sp::Vector3d); break;
...
void Serializer::Handler::processValue(sp::string& value)
{
    if (mode == Mode::Write)
    {
...
void Serializer::Handler::processValue(ISerializable& value)
{
    ...
    if (mode == Mode::Read && read_index >= 0)
    {
        ...
        std::map<sp::string, int> string_lookup_backup(std::move(key_lookup));
...
int Serializer::Handler::addToStringTable(const sp::string& str)
{
